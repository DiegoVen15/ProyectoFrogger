...
namespace game_SDL {
		...
		if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC) < 0) {
			...
			if (mainWindow == NULL) {
				...
				if (renderer != NULL) {
					...
					if (TTF_Init() == -1) {
						cout << "Font init failed\n";
					}
					if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024) == -1) {
						cout << "Sound init failed\n";
					}
	...
	void Renderer::loadBG() {
		SDL_Rect scrnRect = { 0, 0, set->screenWidth, (set->screenHeight / (set->gridSize - 1)) - set->screenHeight / 50 };
	...
	int Renderer::loadMedia() {
		...
		try {
			...
			font = TTF_OpenFont(set->TTFRoboto.c_str(), set->textSize);
			bgm = Mix_LoadMUS(set->mainBGM.c_str());
